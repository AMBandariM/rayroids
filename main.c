...
int main(void) {
    ...
    float shoot_timer = 0.0f; const float shoot_cooldown = 0.3f;
    ...
    while (!WindowShouldClose() && running) {
        ...
        case GS_GAME: {
            ...
            if (shoot_timer > EPSILON) rotation_dir = 0.0f;
            ...
            if (IsKeyPressed(KEY_SPACE) && shoot_timer < EPSILON) {
            ...
            // meteor move
            for (int i = 0; i < n_meteors; ++i) meteors[i].center = Vector2Add(meteors[i].center, Vector2Scale(meteors[i].speed, dt));
            // meteor screen limit
